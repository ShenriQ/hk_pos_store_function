...
exports.assignAdminHandler = ((snap, context, APP_ID) => {
        const channelId = snap.id;
        const channel = snap.data()
        const username = channel.member.name;

        let adminsRef = db.collection(APP_ID + 'Admins');
       return adminsRef.where('online', '==', true).get()
            .then(snapshot => {
                ...
                if (adminToAssign) {
                    ...
                    let channelRef = db.collection(APP_ID + 'Channels').doc(channelId);
                    let messagesRef = db.collection(APP_ID + 'Messages').doc();
